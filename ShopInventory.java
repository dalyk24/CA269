 ...
 */
  ...
  */
 class Milk extends Item implements StorageCondition {
 ...
 public class ShopInventory {
     ...
     public static double billItems(Item[] items, OnSale Sale) {
         ...
         if(Sale != null && Sale.saleCondition(items)) {
            discounted = Sale.reduction(total);
         }

         for(Item m : items) {
            if(m instanceof StorageCondition) {
                System.out.printf("%s (%.1f) (Storage: %s)\n", m.name, m.price, m.storageProcedure());
            }
            else {
         ...
         if(Sale != null && Sale.saleCondition(items)) {
            ...
            System.out.printf("Actual Total: %.1f\n", total);
            System.out.printf("%s\n", Sale.toString());
